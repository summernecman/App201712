...
//                notifyDataSetChanged();
    ...
    public void initTop(UserBean userBean) {
        GlideApp.with(context).asBitmap().centerCrop().placeholder(R.drawable.icon_head1).load(NetValue.获取文件路径("/" + userBean.getHeadurl())).into(bind.imageView2);
        bind.tvName.setText(StringUtil.getStr(userBean.getName()));
        bind.tvPhone.setText(StringUtil.getStr(userBean.getPhone()));
    }
